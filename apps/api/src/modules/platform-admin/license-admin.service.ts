 ...
 * Subscription existant (fundingSource, entitlements) : LicenseAssignment se
 * contente de tracer quelle licence couvre quel abonnement déjà créé par les flux
 * existants (subscriptions/subscription.service.ts). Comme les tranches 3-6, la
 * justification reste optionnelle.
 */
...
export async function assignLicense(
  ...
  if (!subscription) {
    throw new AppError(404, "SUBSCRIPTION_NOT_FOUND", `Subscription not found: ${input.subscriptionId}`);
  }
  ...
  await auditLicenseEntity(
    ...
    { status: updated.status, subscriptionId: input.subscriptionId },
  );
  return updated;
}
