...
Restent hors périmètre de cette tranche : rattachement parent/élève par invitation + code d'activation (§8 — explicitement Phase 8), import/export CSV/Excel, génération de cartes scolaires, transferts inter-établissements (`StudentTransfer`).
...
- **Phase 5 — Gestion de l'établissement** : en cours. Terminé : configuration, utilisateurs, personnel, élèves et inscriptions, documents justificatifs, détection de doublons (voir section dédiée ci-dessus). Restent : rattachement parent/élève (renvoyé à la Phase 8), import/export CSV/Excel, cartes scolaires, transferts inter-établissements.
