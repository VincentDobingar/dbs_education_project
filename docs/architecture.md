...
## Notes et évaluations — §21 en cours (saisie et verrouillage faits)

Nouveau module [grading](../apps/api/src/modules/grading/), monté sur `/api/v1/grading`, même chaîne d'autorisation que les autres (`requireAuth + enforceTenantScope + requireTenantMembership + requirePermission`). Trois permissions dédiées existaient déjà dans le seed sans module pour les consommer (`grades.read`, `grades.write`, `grades.publish` — `TEACHER` a lecture+écriture mais pas publication ; `SCHOOL_OWNER`/`DIRECTOR`/`ACADEMIC_DIRECTOR` ont publication).
...
- **Publication et verrouillage** (`POST /assessments/:id/publish`, derrière `grades.publish`) : marque l'évaluation publiée et verrouille (`isLocked=true`) toutes ses notes déjà saisies en une seule fois — implémentation choisie pour ce verrouillage : au niveau de l'évaluation plutôt qu'au niveau du bulletin (le commentaire du schéma Prisma évoquait la publication du bulletin ; le bulletin n'est pas encore implémenté, voir ci-dessous).
...
**Restent hors périmètre du §21** (prochaine tranche) : calcul des moyennes pondérées par coefficient (matière puis global via `SubjectCoefficient`), classement, bulletins PDF/relevés de notes (`ReportCard`/`ReportCardItem` existent dans le schéma, aucun service ne les pilote encore — même situation que `Program`/`SubjectCoefficient` avant leur tranche dédiée), procès-verbaux et conseils de classe (différés au-delà du MVP), système de notation configurable par tenant (échelle, arrondis, seuil de réussite, mentions — suppose un service `TenantSetting` qui n'existe pas encore, même blocage que la détection de doublons configurable en §19).
...
- **Phase 6 — Gestion académique** : démarrée. §20 (administration académique) terminé — voir section dédiée ci-dessus. §21 (notes et évaluations) en cours : types d'évaluation, évaluations, saisie/verrouillage/correction des notes faits ; moyennes, classement, bulletins PDF restent à faire — voir section dédiée ci-dessus. §22 (présences et discipline) non commencé.
